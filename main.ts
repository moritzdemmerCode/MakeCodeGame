...
function createEnemies () {
    ...
    for (let value5 of tiles.getTilesByType(img`tile4`)) {
        ...
        tiles.setTileAt(value5, img`tile0`)
        bumper.ay = gravity
    ...
    for (let value6 of tiles.getTilesByType(img`tile7`)) {
        ...
        tiles.setTileAt(value6, img`tile0`)
...
scene.onOverlapTile(SpriteKind.Player, img`tile1`, function (sprite, location) {
    info.changeLifeBy(1)
    currentLevel += 1
    if (hasNextLevel()) {
...
function initializeLevel (level: number) {
    ...
    playerStartLocation = tiles.getTilesByType(img`tile6`)[0]
    tiles.placeOnTile(hero, playerStartLocation)
    tiles.setTileAt(playerStartLocation, img`tile0`)
    createEnemies()
    spawnGoals()
}
...
function spawnGoals () {
    for (let value7 of tiles.getTilesByType(img`tile5`)) {
        ...
        tiles.setTileAt(value7, img`tile0`)
    }
}
